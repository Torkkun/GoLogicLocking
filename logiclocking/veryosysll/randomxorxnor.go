...
func RandomXorXnor(ctx context.Context, driver neo4j.DriverWithContext, dbname string, keylen int) (map[string]bool, error) {
	...
	for i, indxnum := range rnum {
		...
		newelementid, err := veryosys.CreateRandomLLGateNode(ctx, driver, dbname, newL)
		if err != nil {
			log.Println(err)
			return nil, err
		}
		...
		newinleyid, err := veryosys.CreateInOutNode(ctx, driver, dbname, newio)
		if err != nil {
			log.Println(err)
			return nil, err
		}
		...
		// Relation削除
		if err := veryosys.DeleteConnection(ctx, driver, dbname, relationId); err != nil {
			log.Println(err)
			return nil, err
		}
		// xor/xnorと親ノードを接続
		if err := veryosys.CellConnection(ctx, driver, dbname, &veryosys.ConnectionPair{
...
func RandomXorXnorTxTest(ctx context.Context, driver neo4j.DriverWithContext, dbname string, keylen int) (map[string]bool, error) {
	pair, err := veryosys.GetRelationshipsAndPairNodes(ctx, driver, dbname)
	if err != nil {
		return nil, err
	}
	...
	// 明示的なTransactionを開始
	_, err = veryosys.Transaction(ctx, driver, dbname, func(tx neo4j.ExplicitTransaction) (interface{}, error) {
		for i, indxnum := range rnum {
			b, err := utils.Choice([]bool{true, false})
			if err != nil {
				return nil, err
